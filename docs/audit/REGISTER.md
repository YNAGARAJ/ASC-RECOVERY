...
|---|---|---|---|---|---|---|---|---|
| F-01 | **CRITICAL** | Money | `ingestion/plan.py:86-100`, `ingestion/apply.py:136-159` | Reversal netting can silently drop the offsetting shortfall (line-count mismatch) or attach to the wrong service line (index reused, procedure differs) — overstates recovered dollars on a real claim. | Match the reversing finding to the *original* claim's service line, not the reversal claim's line positions; never silently drop a reversing finding — fail loudly or quarantine instead. Add a test: `sum(shortfall)==0` after original+reversal with mismatched line counts. | M | Blocks any real pilot (Ph.12) with a real reversal in the file | **FIXED (824b26a)** — re-verified during Phase 9 (`docs/MASTER-BUILD-PROMPT-V2.md`, audit amendment) and found **half-unfixed**: the line-count-mismatch/misattribution half was genuinely fixed (`service_line_id` carried through from the original finding, proven by `tests/ingestion/test_plan.py` and end-to-end by `tests/db/test_reversal_netting.py::test_reversal_with_fewer_lines_than_original_nets_every_finding_to_zero`), but the amendment's second, explicit requirement — "never silently drop a reversing finding — fail loudly or quarantine instead" — was never implemented: a reversal claim whose control number matched **zero** prior findings silently produced `findings=()` and ingested successfully, structurally indistinguishable from a legitimately zero-variance original claim. Closed for real this phase: `ingestion/plan.py::build_ingestion_plan` now quarantines the whole file when any reversal claim finds no matching prior findings, naming the unmatched control number(s) in the diagnostic (`tests/ingestion/test_plan.py::test_unmatched_reversal_quarantines_the_file`). |
...
| F-16 | HIGH | Domain/Money | `domain/contract.py:19-21,182-194` | `BilateralConvention.TWO_LINE_SPLIT` is a storable enum value with **no pricing implementation** — a contract configured with it silently prices both lines at 100% (no bilateral reduction at all), a direct money error. | Implement the two-line-split pricing branch, or reject the value at contract-version creation until implemented. | M | — | **FIXED (43e241b)** — re-verified during Phase 8 (`docs/MASTER-BUILD-PROMPT-V2.md`, audit amendment) and found still **half-unreachable**: the domain-layer pricing branch was real, but `api/schemas.py::BilateralRuleIn` had no `convention` field at all, and `api/repository.py::_rule_input_to_contract_version` hardcoded `BilateralConvention.SINGLE_LINE_150_PCT` unconditionally — the only production entry point could never actually select `TWO_LINE_SPLIT`, satisfying "storable enum value with no way to store it" in spirit even though the pricing branch itself worked. Closed for real this phase: `BilateralRuleIn.convention` added, threaded through `api.repository.RuleInput`/`ContractVersionInput`, proven end to end against a live Postgres in `tests/api/test_contracts_live_db.py` (a contract created via `POST /contracts/{id}/versions` with `convention: "two_line_split"` now actually prices a two-line bilateral claim with the 100%/remainder split, not just at the domain-object level). |
...
| F-20 | HIGH | Deploy/Security | `security/kms_env.py:32-58`, `main.py:96-100`, both clouds' `secrets_and_kms.tf` | No `AwsKmsAdapter`/`AzureKeyVaultAdapter` exists — the app's own PHI-envelope encryption, wherever it runs, uses a static in-secret KEK (`EnvKMS`) that never rotates automatically, with no per-operation KMS audit trail, while both clouds provision a real, rotating KMS key the app never calls. | Implement both cloud adapters behind the existing `KeyManagementService` port; requires a real cloud account to verify. | L | Related to B-20 (KEK rotation on the stopgap adapter) | **FIXED (02c6e38)** — `AwsKmsAdapter`/`AzureKeyVaultAdapter` (`security/kms_aws.py`, `security/kms_azure.py`) implemented behind the existing `KeyManagementService` port, wired into `main.py` via a new opt-in `KMS_PROVIDER` env var (unset/`"env"` keeps `EnvKMS` as the unchanged default). Unit-tested against fake SDK clients (`tests/security/test_kms_aws.py`, `test_kms_azure.py`) and `main.py`'s new branches (`tests/test_main.py`), by user decision, since neither cloud account exists in this environment to verify against for real — same disclosure pattern as every other real-cloud adapter in this codebase. Documented in `docs/RUNBOOK.md`'s "Key rotation" section. |
| F-21 | HIGH | Deploy | `docs/RUNBOOK.md:128-151`, `docs/compliance/README.md:28` | Backup/restore is documented but has never been executed or timed against a real database — the Phase 9 gate ("restore rehearsed and timed") is unmet. *(Absorbs the broader spec-conformance finding "deployment gate entirely unexecuted" — this is its most concrete evidence.)* | Execute the documented restore into a throwaway instance once a database exists; record real wall-clock time. Operational drill, no code change. | M | Blocks Ph.9 gate | **PARTIALLY ADDRESSED (7397b3d) — stays OPEN, by user decision.** `scripts/db/restore_drill.sh` automates the documented procedure (restore, wait, verify `alembic_version`/row counts/RLS, print elapsed time, tear down) for either cloud, but has never been run against a real AWS or Azure account — no such account exists in this environment. The core requirement (execute for real, record the actual wall-clock number) genuinely cannot be satisfied here; this closes "the procedure only exists as prose," not the finding itself. |
| F-22 | HIGH | Test quality | `tests/api/test_pilot_workflow_live_db.py`, `tests/db/*`, `tests/ingestion/*_live_db.py` | Every cross-seam (domain→db→api) integration test is gated behind `TEST_DATABASE_URL`. In every environment without a live Postgres — including this one — the runnable 408-test suite proves no integration behavior at all; a real schema/repository regression would stay green locally. | No code fix — an infrastructure gap. Get a live Postgres into CI (already true) and, ideally, a lightweight local option (e.g. a Docker-based dev-container Postgres) so this isn't Postgres-only in one environment. | — | Same underlying gap as F-01's non-local-verifiability | **TRIAGED, stays OPEN.** CI already runs the full live-DB suite against a real Postgres 16 service container (`.github/workflows/ci.yml`), and `docker-compose.yml` + `docs/DB_SETUP.md` already document a local option — both halves of this row's "Fix" predate Wave 3. This session discovered a real PostgreSQL 18 server already running as a Windows service in the dev environment (not the Docker path DB_SETUP.md assumes), which would let the live-DB suite actually run here for the first time — user confirmed doing this, but didn't have the local `postgres` superuser password on hand and deferred the setup to a future session. Exact `CREATE ROLE`/`CREATE DATABASE` commands to hand off are in the conversation; picking this up doesn't need re-deriving them. |
| F-23 | HIGH | Process | (whole codebase; see F-01 through F-22) | The Phase 10 gate ("adversarial review shows zero HIGH/CRITICAL") does not reflect the current tree — this audit independently found the HIGH findings above in shipped code. Either the adversarial-reviewer subagent was not re-run post-Phase-12, or it was and these weren't surfaced/fixed. | Re-run the `adversarial-reviewer` subagent on HEAD once F-01 through F-22 are triaged; don't treat Phase 10 as passed until it shows clean or every finding is formally accepted. | S | Depends on F-01–F-22 being triaged first | **FIXED** — `adversarial-reviewer` subagent re-run against HEAD (all of F-01–F-22 triaged first, per the dependency). Result: **zero HIGH/CRITICAL findings.** Money, date-of-service pricing, tenant isolation (RLS forced, no bypass role), audit-log coverage, PHI redaction, authorization, and this session's new cloud-KMS adapters all held up. 6 LOW items noticed in passing logged as B-60 through B-65 (backlog, below) — none gate this finding. |

---

## BACKLOG (all MEDIUM and LOW — 65 rows)
...
### F-23 re-review findings (post Wave 3 remediation — `adversarial-reviewer` subagent run against HEAD, found zero HIGH/CRITICAL; these are what it noticed in passing)

| ID | Sev | File:line | What breaks (brief) |
|---|---|---|---|
| B-60 | LOW | `scripts/db/restore_drill.sh:122,127,137` | Restored-instance connection string (contains the master/owner password) is passed to `psql` as an argv argument — visible via `ps`/`/proc` to any other user on the host running the drill |
| B-61 | LOW | `scripts/ingestion/poll_remittances.py:146-151` | Poller prints the raw remittance filename to stdout unredacted — a payer-supplied filename could carry an identifier, and stdout is outside `PHIRedactionFilter`'s path |
| B-62 | LOW | `ingestion/pipeline.py:87` | Span attribute `source` is the raw source string on the poll path — `PHIScrubbingSpanExporter` only catches SSN/MBI-shaped substrings, not an arbitrary identifier embedded in a filename |
| B-63 | LOW | `security/kms_aws.py:39-47` | `unwrap_key` passes `KeyId=kek_id` to `Decrypt` — correct for AWS's own automatic rotation, but a *manual* re-key (repointing the alias to a genuinely new key) makes prior ciphertexts fail with `IncorrectKeyException` instead of resolving from the blob |
| B-64 | LOW | `api/repository.py:892-911` | `decide_packet` writes `recovery_packets` (whose `draft_text` contains rendered PHI) with `phi_accessed=False` — the write is audited (CLAUDE.md rule 5 satisfied) but the flag understates the resource's PHI sensitivity |
| B-65 | LOW | `scripts/onboard_customer.py`, `api/routes/auth.py:110` | First admin is created with `password_hash=None`/`mfa_secret_encrypted=None`; no endpoint or script anywhere provisions a password or enrolls MFA for that account — it fails closed at login (safe), but the bootstrap-to-first-login path is incomplete |
