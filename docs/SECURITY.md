...
- **Per-org data residency.** The last of `docs/MASTER-BUILD-PROMPT-V2.md`'s
  Phase 6 items — a per-org rate-limiting ceiling, forced re-auth for
  PHI export, and per-org encryption keys (BYOK-ready) are all now built
  (see their own rows above). Data residency remains not started; given
  this build is one shared Postgres instance in one region today, it
  would realistically mean a stored, honest preference/contractual flag
  rather than physical multi-region enforcement — worth confirming that
  framing before building it, not assuming.
