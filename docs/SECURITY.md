...
| Per-org encryption keys (BYOK-ready) | §164.312(a)(2)(iv), §164.308(a)(5)(ii)(D) | `organizations.kms_key_id` (nullable, `alembic/versions/0010_org_kms_key.py`) + `EnvelopeEncryptor.encrypt`'s optional `kek_id` override + `ingestion/pipeline.py`/`ingestion/apply.py` resolving and threading an org's dedicated key through patient-PHI encryption at write time (`docs/MASTER-BUILD-PROMPT-V2.md` Phase 6). `AwsKmsAdapter`/`AzureKeyVaultAdapter`'s `wrap_key` now accept any `kek_id`, not just their own configured default — the restriction removed was self-imposed, not a reflection of anything the real cloud APIs require. `decrypt` needed no changes: `EncryptedPayload.kek_id` already travels with each ciphertext, so a payload always decrypts under whichever key actually wrapped it regardless of an org's "current" key | Implemented, tested (`tests/security/test_encryption.py`, `test_phi_columns.py`, `test_kms_aws.py`, `test_kms_azure.py`; DB-backed: `tests/ingestion/test_apply_org_kms_key.py`, `tests/db/test_organization_kms_key.py`, both skip without a live Postgres). **Deliberately incompatible with `EnvKMS`** — it holds exactly one static key and raises `KeyError` for any other `kek_id`; setting `kms_key_id` on an org before `KMS_PROVIDER` is `aws-kms`/`azure-keyvault` breaks ingestion for that org's claims (loudly, not silently) — `docs/RUNBOOK.md`'s "Per-org encryption keys (BYOK)" section is explicit about the ordering. No self-service API for setting this column, by design — a direct, operator-reviewed DB write only (same reasoning as `scripts/onboard_customer.py` itself: the blast radius of misconfiguring an org's encryption key is too high for a self-service endpoint) |
...
| Per-org data residency declaration | §164.308(a)(1)(ii)(A) (risk analysis / data governance, by extension), customer-questionnaire relevance | `org_policies.data_residency_region` (`alembic/versions/0011_org_data_residency.py`) + `GET`/`PUT /org-policy` (`docs/MASTER-BUILD-PROMPT-V2.md` Phase 6, the last item of this phase's checklist) | Implemented, tested (`tests/api/test_org_policy.py`, `tests/db/test_org_policy.py`). **A stored declaration, not a technical control** — this system runs one shared Postgres instance in one region today, so there is no per-org routing, replication, or physical placement this column drives; free-text, not validated against real infrastructure, because none of the multi-region kind exists to validate against. Unlike per-org encryption keys (`organizations.kms_key_id`, row above), this field is deliberately self-service — misdeclaring it doesn't lock an org out of its own data, so it doesn't need the same operator-only treatment. If this platform ever adds real multi-region deployment, this column is the source of truth to build routing against, not a promise already kept |
| Async job queue: PHI-payload encryption + RLS scoping | §164.312(a)(2)(iv), §164.312(a)(1) | `jobs.payload_encrypted` (`alembic/versions/0012_jobs.py`, `docs/MASTER-BUILD-PROMPT-V2.md` Phase 7 — "nothing heavy may run inside an HTTP request") reuses the exact `security/phi_columns.py::encrypt_phi_field`/`decrypt_phi_field` machinery `claims.patient_name_encrypted` already uses, including per-org BYOK key resolution — no new crypto for a job's raw 835 file content. `jobs` carries the same `facility_access` RLS policy shape every facility-scoped table has had since 0001, and the worker re-establishes the *job's own submitter's* resolved access via `access_session(session_factory, job.user_id)` to execute it — a revoked membership takes effect on a not-yet-run job exactly like it already does everywhere else | Implemented, tested (`tests/db/test_jobs_queue.py`'s RLS-outsider proof; `tests/jobs/test_runner_live_db.py`'s end-to-end enqueue→claim→complete proof, both skip without a live Postgres). `GET /jobs`/`GET /jobs/{id}` responses (`api.repository.JobSummary`) have no field for `payload_encrypted` at all — structurally incapable of leaking it, not just filtered out |
| Dead letter queue with alerting (async job failures) | §164.308(a)(6)(ii) (security incident procedures/response) | `db.repository.fail_job` transitions a job to `dead_letter` once `attempts >= max_attempts` (exponential backoff between attempts, `src/jobs/runner.py::_backoff_seconds`); `observability/alerts.py::evaluate_job_dead_lettered_alert` fires on that exact transition via the same `NotificationPort` every other alert in this codebase uses, plus a `job_dead_lettered` `audit_log` entry | Implemented, tested (`tests/observability/test_alerts.py`, DB-backed dead-letter transition proof in `tests/db/test_jobs_queue.py`). Errors are `security/redaction.py::scrub_text`'d before storage — CLAUDE.md rule 6, never a raw exception message that might echo PHI |
| 837 claim file ingestion: diagnosis-code encryption + RLS scoping | §164.312(a)(2)(iv), §164.312(a)(1) | `claims.diagnosis_codes_encrypted` (`alembic/versions/0014_claim_files_and_837_enrichment.py`, `docs/MASTER-BUILD-PROMPT-V2.md` Phase 9 — "also ingest 837 claim files where available") reuses the exact `security/phi_columns.py::encrypt_phi_field`/`decrypt_phi_field` machinery `patient_name_encrypted` already uses, including per-org BYOK key resolution — no new crypto. Diagnosis codes are treated as PHI (health condition data tied to a specific claim/patient) even though the new `rendering_provider_name`/`service_lines.units` columns added alongside them are not (provider identity/a bare quantity, not a patient identifier — plain columns, same precedent `patient_control_number` already sets). `claim_files` (the 837 idempotency/audit-tracking table, deliberately separate from `remittances` — an 837 isn't a remittance) carries the same `facility_access` RLS policy shape every facility-scoped table has had since 0001. Diagnosis codes reach the packet-drafting LLM prompt only as a placeholder token (`packets/prompt.py::DIAGNOSIS_CODES_TOKEN`), never literal text — same F-13 treatment claim reference/date of service already get | Implemented, tested (`tests/domain/test_x837.py`'s pure parser suite; `tests/ingestion/test_837_live_db.py`'s enrichment/unmatched-claim proofs, skip without a live Postgres; `tests/packets/test_prompt.py`'s PHI-never-literal proof). Discovered and fixed alongside this: `packets/currency.py`'s hallucination-detection regex could false-positive on an ICD-10 code's own internal decimal point (e.g. `E11.9`) — `extract_currency_figures`'s `exclude` now masks known-safe substrings out of the text before matching, rather than filtering matched fragments after the fact, closing the gap for any future excluded value shaped like `E11.9`, not just today's diagnosis codes |
